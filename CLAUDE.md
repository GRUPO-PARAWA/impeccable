...
Two facts about the org upload are documented rather than inferred: the file must be a valid `.zip` under **50 MB** (ours are ~1.6 MB), and plugin names must be **lowercase words separated by hyphens** (`impeccable` qualifies). The structural requirements are not documented, and the org validator is known to be stricter than the CLI in ways the CLI cannot catch (claude-code issue #63081 rejects angle brackets in a `SKILL.md` description that `claude plugin validate` accepts). So when an upload is rejected, prefer evidence over another round of packaging changes.

**`impeccable-plugin.zip`, `impeccable-plugin-wrapped.zip`, and `impeccable-skill.zip` are tracked at the repo root**, one per upload shape, because the operator gets them by downloading a single file from GitHub. The wrapped variant exists only because the unwrapped shape was rejected and a wrapper directory is the last structural difference worth testing; delete it once one shape is confirmed to work. An artifact that only exists in gitignored `dist/` is not reachable that way, which is how the first attempt ended up uploading `Code -> Download ZIP` instead. They are generated binaries: refresh both with `npm run build:upload-zips` after any skill change, never by re-zipping a directory. Two 1.6 MB blobs per refresh do accumulate in history; if that becomes a problem, move them to release assets and update the README table.
